...
def test_run_research_happy_path(monkeypatch):
    ...
    monkeypatch.setattr(
        pipeline,
        "serp_search",
        lambda q, engine="google", api_key=None: {"engine": engine, "q": q},
    )
    monkeypatch.setattr(
        pipeline,
        "reddit_search_api",
        lambda keyword, api_key=None, dataset_id=None: {
            "parsed_posts": [{"url": "https://r/test"}]
        },
    )
    monkeypatch.setattr(
        pipeline,
        "reddit_post_retrieval",
        lambda urls, api_key=None, comments_dataset_id=None: {
